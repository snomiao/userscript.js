...
(async() => {
    ...
    if (location.href.startsWith("https://m.jr.jd.com/mjractivity/rn/rn_bt_raise/index.html")) {
        var 取按钮 = async(按钮名称) => (await 直到(() => [...document.querySelectorAll("div>span")].filter(e => e.innerText == 按钮名称)[0])).parentElement;
        点击元素(await 取按钮("提升额度"));
        var re = Promise.race([
