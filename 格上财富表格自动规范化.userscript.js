    ...
    var waitJQ_for = function(func, interval = 1000){
        if(typeof(jQuery) == "undefined"){
            document.documentElement.appendChild(
                document.createElement('script') ).src='https://code.jquery.com/jquery-3.3.1.js';
    ...
    var start = function(){
        waitJQ_for(function(){
            ...
            if($srcEle1.length || $srcEle2.length || $srcEle3.length){
                Array.from($srcEle1).map(
                    (ele, i) => replace_element_tag($srcEle1.slice(i, i + 1), "td"));
                Array.from($srcEle2).map(
                    (ele, i) => replace_element_tag($srcEle2.slice(i, i + 1), "tr"));
                Array.from($srcEle3).map(
                    (ele, i) => replace_element_tag($srcEle3.slice(i, i + 1), "table"));
