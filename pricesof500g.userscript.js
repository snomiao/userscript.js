...
<span class="priceof500g" style="background: ${颜色}; color: white" title="${描述}">
    ${每千克价格按每斤解释(每千克价格)}
        ...
        价格标签.商品信息 = 商品信息;
        // 标记商品已处理
        商品信息.元素?.classList.add('pricesof500g-mark');
        // 标签换新或显示
...
| taobao.com | .J_TSaleProp>li.tb-selected | a                     | 全局>#J_PromoPriceNum  | 小的商品描述标签，
...
| tmall.com  | .J_TSaleProp>li.tb-selected | a                     | 全局>.tm-promo-price   | 小的商品描述标签
...
function 价格标签探索() {
    ...
    var 未解释商品标签列 = [
        ...document.querySelectorAll('.J_TSaleProp>li:not(.pricesof500g-mark)'),
    ];
    ...
    var 剩余未解释商品标签点击 = () => {
        ...
        if (未解释商品标签) {
            ...
            setTimeout(主动刷新函数, 1);
            setTimeout(剩余未解释商品标签点击, 16);
        } else {
            console.log('未解释商品标签处理完成');
        }
    };
    剩余未解释商品标签点击();
}
