    ...
    const handler = (event) => {
        ...
        mapObjIndexed((fn, hotkey) => {
            const conds = `${mods}+${hotkey.toLowerCase()}`
                ...
                .map((k, i) => [
                    k,
                    Boolean(i >= 4) === Boolean(event[`${k}Key`]),
                ]);
            if (
                !Object.entries(Object.fromEntries(conds)).every(([, ok]) => ok)
            )
                return;
...
function assocPathFn(path2, newValue, input) {
    const pathArrValue =
        typeof path2 === "string"
            ? path2
                  .split(".")
                  .map((x) => (isInteger(Number(x)) ? Number(x) : x))
            : path2;
    ...
    return pipe(
        ...
        (list) =>
            list.flatMap((father, i, a) =>
                !a.some(
                    (son, j) => i != j && father?.element.contains(son.element)
                )
                    ? [father]
                    : []
            ),
