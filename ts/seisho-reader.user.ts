...
(function () {
  ...
  async function main() {
    ...
    function $$(selector = "*", parent: HTMLElement = document.body) {
    ...
    while (instance === globalThis.seishoReader) {
      try {
        ...
        //: zip2(b, a).map((e) => e.reverse());
        const cols = [...$$(".bible-api .scripture-styles")] as [
          HTMLDivElement,
          HTMLDivElement
        ];
        const sc = cols.map((ss) => {
          const e = ss.cloneNode(true) as HTMLDivElement;
          return (
            $$(".v,p.s1", e).map(
              (e) => (e.textContent = ((s) => `\n${s} `)(e.textContent))
            ),
        ...
        for await (const e of nn) {
          ...
          const [head, sentext] = e.split(">>").map((e) => e.trim());
          ...
          while (speechSynthesis.speaking) {
            if (+new Date() - t0 > 30e3) break;
            await new Promise((r) => setTimeout(r, 100));
          }
          speechSynthesis.cancel();
          speechSynthesis.speak(uttr);
        }
        const nextbtn = $$("a.next")[0] as undefined | HTMLButtonElement;
